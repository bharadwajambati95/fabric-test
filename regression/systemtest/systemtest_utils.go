...
func ChangeTargetPeers(testFilePath, testFileName, targetPeersType string) error {
	...
	var err error
	yamlFile, err := ioutil.ReadFile(filepath.Join(testFilePath, testFilePath))
	if err != nil {
		return err
	}
	...
	err = ioutil.WriteFile(filepath.Join(testFilePath, fmt.Sprintf("%s_%s", strings.ToLower(targetPeersType), testFileName)), yamlBytes, 0644)
	if err != nil {
		return err
	}
	return err
